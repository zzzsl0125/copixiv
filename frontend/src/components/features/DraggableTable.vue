...
import { RefreshCw, GripVertical } from '@lucide/vue'
...
const emit = defineEmits<{ (e: 'reorder', items: unknown[]): void }>()
            ...
            <tr class="hover:bg-gray-50 transition-colors group">
              ...
              >
                <slot :name="col.key" :item="element" :index="index">
                  <span class="text-sm text-gray-900">{{ (element as Record<string, unknown>)[col.key] }}</span>
                </slot>
              </td>
            </tr>
          </template>
