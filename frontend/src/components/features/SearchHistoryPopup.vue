...
<template>
  <div v-if="show && history.length > 0" class="absolute top-full mt-2 w-full max-w-md bg-white border border-gray-200 rounded-md shadow-lg z-20">
    <div class="px-4 py-2 flex justify-between items-center border-b border-gray-200">
      ...
      <button @click.stop="emit('clear-all')" class="text-xs text-blue-500 hover:text-blue-700">
        全部清除
      </button>
    </div>
    <ul class="py-1 max-h-60 overflow-y-auto">
      ...
      >
        ...
        <button @click.stop="emit('delete-item', item.id)" class="text-gray-400 hover:text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity">
          <X class="w-4 h-4" />
        </button>
      </li>
    </ul>
