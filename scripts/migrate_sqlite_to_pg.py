...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        # The migration inserts explicit tag.id values (the greenfield baseline
        # uses GENERATED BY DEFAULT AS IDENTITY to allow that).  Explicit
        # inserts do NOT advance the identity sequence, so without a sync the
        # next auto-generated id would collide with an existing row.  Advance
        # the sequence past the highest migrated id.  (bugfix: postgres-migration)
        cur.execute(
