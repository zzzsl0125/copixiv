...
async def _run_task(session_factory, tmp_path, *, operation, novel_ids,
    ...
    from copixiv.tasks import batch

    original = batch.BATCH_TASK_SAFETY_CHUNK
    batch.BATCH_TASK_SAFETY_CHUNK = chunk_size
    try:
        task_id = None
        if with_history:
        ...
        batch.BATCH_TASK_SAFETY_CHUNK = original
...
class TestBatchExportTask:
    def test_builds_zip_into_download_dir_and_download_endpoint_serves_it(
        ...
        from copixiv.db.uow import SqlUnitOfWork
        from copixiv.db.write_lock import DbWriteLock
        from copixiv.tasks.batch import BatchExportArgs, batch_export
