...
class TestBeginTransactionSemantics:
    ...
    async def test_uow_owns_its_session(self, session_factory):
        """Single mode: UoW creates its session from the factory and owns it."""
        uow = SqlUnitOfWork(session_factory)
        session = uow.session  # created lazily from the factory
        async with uow.begin():
            uow.session.add(Author(author_id=3, author_name="carol"))

        assert uow._session is None  # begin() closed the session it owns
        with session_factory() as s:
            assert s.get(Author, 3) is not None

    async def test_owned_session_closed_after_begin(self, session_factory):
        """begin() closes the UoW's session for the next cycle."""
        uow = SqlUnitOfWork(session_factory)
        async with uow.begin():
