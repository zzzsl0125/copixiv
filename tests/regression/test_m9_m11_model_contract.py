...
- 工厂函数返回 NovelDraft（frozen dataclass，写路径载体，带瞬态字段）
...
from copixiv.core.draft import (
    build_novel, build_from_novel_info, build_from_webview, NovelDraft,
)
...
def test_build_novel_returns_novel_draft():
    novel = build_novel(id=1, title="t", author_id=2)
    assert isinstance(novel, NovelDraft)
    # 瞬态字段在载体上
    ...
    assert "tag" not in novel2.__dict__


def test_build_from_novel_info_returns_draft_with_epub_none():
...
def test_novel_roundtrip_via_instance_dict():
    """仓储可安全地用 __dict__ 拿字段（transient 字段也在，但按白名单过滤）。"""
