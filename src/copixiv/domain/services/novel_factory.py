...
def build_from_webview(data: Any, download_dir: str = "download") -> dict[str, Any]:
    ...
    return build_novel_dict(
        ...
        author_name=None,
        like=safe_get(data.rating, "bookmark", 0) if data.rating else 0,
        view=safe_get(data.rating, "view", 0) if data.rating else 0,
        text=len(data.text),
...
def build_from_novel_info(data: Any, download_dir: str = "download") -> dict[str, Any]:
    ...
    return build_novel_dict(
        ...
        caption=data.caption,
        series_id=safe_get(series, "id") if series else None,
        series_name=safe_get(series, "title") if series else None,
        series_index=safe_get(series, "index") if series else None,
