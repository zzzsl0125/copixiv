...
Implements the ``send_task_result``/``close``/``name`` contract that the
task kernel expects (formerly the ``NotifierPort`` protocol, now removed):
one backend failing (network error, misconfigured URL) must never affect
the others, and ``close()`` releases every backend.
"""
...
class CompositeNotifier:
    ...
    def __init__(self, backends: list[TelegramNotifier | WebhookNotifier]):
        self._backends = list(backends)

    @property
    def backends(self) -> list[TelegramNotifier | WebhookNotifier]:
